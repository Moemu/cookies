...
test('short drama preroll V4 uses one selected reference board and persistent workspace commands', async () => {
  ...
  assert.match(workspace, /reference_board_batch/)
  assert.match(workspace, /api\.generateShortDramaReferenceBoards/)
  assert.match(workspace, /api\.selectShortDramaReferenceBoard/)
  ...
  assert.match(workspace, /Prompt \+ 单张宫格参考/)
  assert.match(workspace, /生成新一批/)
  assert.match(workspace, /智能改写并补生成/)
