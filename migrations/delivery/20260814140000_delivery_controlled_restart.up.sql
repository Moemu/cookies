...
ALTER TABLE delivery_platform_entity_mappings
  ...
  ADD CONSTRAINT chk_delivery_platform_mapping_state_hash CHECK (
    ...
    OR (
      current_state_action IN ('update_promotion_budget', 'update_promotion_schedule', 'update_promotion_materials', 'pause_promotion', 'resume_promotion')
...
ALTER TABLE delivery_platform_entity_mapping_revisions
  ...
  ADD CONSTRAINT chk_delivery_platform_mapping_revision_state_hash CHECK (
    ((previous_state_action IS NULL AND previous_state_hash IS NULL) OR (previous_state_action IN ('update_promotion_budget', 'update_promotion_schedule', 'update_promotion_materials', 'pause_promotion', 'resume_promotion') AND previous_state_hash REGEXP '^[0-9a-f]{64}$'))
    AND ((current_state_action IS NULL AND current_state_hash IS NULL) OR (current_state_action IN ('update_promotion_budget', 'update_promotion_schedule', 'update_promotion_materials', 'pause_promotion', 'resume_promotion') AND current_state_hash REGEXP '^[0-9a-f]{64}$'))
  );
