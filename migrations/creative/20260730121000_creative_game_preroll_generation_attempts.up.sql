CREATE TABLE creative_game_preroll_generation_attempts (
  ...
  PRIMARY KEY (id),
  UNIQUE KEY uq_creative_game_preroll_attempt_provider_job (organization_id, provider_job_id),
  KEY idx_creative_game_preroll_attempt_task (organization_id, project_id, task_id, created_at),
  CONSTRAINT fk_creative_game_preroll_attempt_task
    FOREIGN KEY (organization_id, task_id) REFERENCES creative_tasks(organization_id, id),
  CONSTRAINT chk_creative_game_preroll_attempt_draft_revision CHECK (draft_revision >= 1),
  CONSTRAINT chk_creative_game_preroll_attempt_output CHECK (
