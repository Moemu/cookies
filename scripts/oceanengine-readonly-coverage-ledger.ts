...
export type CoverageTarget = 'account_context' | 'project_list' | 'promotion_list' | 'project_detail_or_edit' | 'promotion_detail_or_edit' | 'material_relation_summary' | 'report_overview'
type PageTarget = Exclude<CoverageTarget, 'account_context'>
type PageKind = PageTarget | 'unknown'
type CheckState = 'passed' | 'failed' | 'not_run' | 'not_applicable'
type CoverageStatus = 'confirmed' | 'blocked' | 'not_accessible'
...
function coveragePaths(localAppData = process.env.LOCALAPPDATA) {
  if (!localAppData) throw new Error('LOCALAPPDATA is not set')
  const root = resolve(localAppData, 'cookies', 'browser-rpa', 'calibration')
  return {
    root,
    before: join(root, 'coverage-before.json'),
    after: join(root, 'coverage-after.json'),
...
export function classifyCoveragePage(observations: LocatorObservation[], editMarkerPresent = false): PageKind {
  ...
  if (editMarkerPresent && present('promotion_name') && present('save_and_close')) return 'promotion_detail_or_edit'
  if (editMarkerPresent && present('project_marketing_purpose') && present('project_name') && present('project_deep_optimization')) return 'project_detail_or_edit'
...
async function inspectPage(page: Page, phase: CoverageObservation['phase'], expectedAccountHash: string): Promise<CoverageObservation> {
  ...
  const pageKind = classifyCoveragePage(observations, editMarkerPresent)
  const pageState = {
    pageKind,
    observations,
...
async function observeCommand(localAppData: string, phase: CoverageObservation['phase']) {
  ...
  await atomicJSON(phase === 'before' ? paths.before : paths.after, observation)
  return { outcome: 'observed' as const, phase, page_kind: observation.page_kind, evidence_sha256: sha256(canonicalJSON(observation)) }
}
...
const requirements: Record<PageTarget, { locators: LocatorKey[]; structure: (observation: CoverageObservation) => boolean }> = {
  ...
  project_detail_or_edit: { locators: ['project_marketing_purpose', 'project_name', 'project_deep_optimization'], structure: item => item.structure.textbox_count > 0 },
  promotion_detail_or_edit: { locators: ['promotion_name', 'save_and_close'], structure: item => item.structure.textbox_count > 0 && item.edit_marker_present },
...
export function buildCoverageLedger(before: CoverageObservation, after: CoverageObservation, manifestFieldCount: number): CoverageLedger {
  ...
  const targets: CoverageLedger['targets'] = [{
    target: 'account_context', attempted: true,
    status: sameAccount ? 'confirmed' : 'blocked',
  ...
  for (const target of Object.keys(requirements) as PageTarget[]) {
    ...
    const confirmed = httpsPassed && pageTypePassed && sameAccount && structurePassed && uniquePassed && sameState
    targets.push({
      target, attempted: true, status: confirmed ? 'confirmed' : 'blocked', reason: confirmed ? 'current_page_confirmed' : 'verification_failed',
      ...
      drift_state: confirmed ? 'stable' : 'page_drift', locators,
    })
  }
  ...
  const currentPageTarget = before.page_kind !== 'unknown' && before.page_kind === after.page_kind ? before.page_kind : undefined
...
type LegacyCoverageLedger = Omit<CoverageLedger, 'page_evidence' | 'no_write_proof' | 'evidence_sha256'> & {
  before: EvidenceSnapshot
  after: EvidenceSnapshot
  no_write_proof: PageNoWriteProof
  evidence_sha256: string
...
function upgradeLegacyLedger(legacy: LegacyCoverageLedger): CoverageLedger {
  const pageTarget = legacy.targets.find(item => item.target !== 'account_context' && item.status === 'confirmed')?.target as PageTarget | undefined
  const evidenceWithoutHash = pageTarget ? { target: pageTarget, before: legacy.before, after: legacy.after, no_write_proof: legacy.no_write_proof } : undefined
  const pageEvidence: PageEvidence[] = evidenceWithoutHash ? [{ ...evidenceWithoutHash, evidence_sha256: sha256(canonicalJSON(evidenceWithoutHash)) }] : []
  const targets = legacy.targets.map(item => item.target === pageTarget && pageEvidence[0] ? { ...item, page_evidence_sha256: pageEvidence[0].evidence_sha256 } : item)
  const ledgerWithoutHash = {
    schema_version: coverageSchemaVersion,
    observed_at: legacy.observed_at,
    platform: legacy.platform,
    allowed_host: legacy.allowed_host,
    scope: legacy.scope,
    completion_state: legacy.completion_state,
    source_inventory: legacy.source_inventory,
    account_context_sha256: legacy.account_context_sha256,
    targets,
    rejected_locators: legacy.rejected_locators,
    page_evidence: pageEvidence,
...
export function mergeCoverageLedgers(previous: CoverageLedger, current: CoverageLedger): CoverageLedger {
  ...
  const targets = current.targets.map(item => {
    ...
    return evidence ? { ...selected, page_evidence_sha256: evidence.evidence_sha256 } : selected
  })
...
async function finalizeCommand(localAppData: string) {
  const paths = coveragePaths(localAppData)
  const before = JSON.parse(await readFile(paths.before, 'utf8')) as CoverageObservation
  const after = JSON.parse(await readFile(paths.after, 'utf8')) as CoverageObservation
  ...
  try {
    const stored = JSON.parse(await readFile(paths.ledger, 'utf8')) as CoverageLedger | LegacyCoverageLedger
    const previous = 'page_evidence' in stored ? stored : upgradeLegacyLedger(stored)
