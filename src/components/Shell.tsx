...
export function Shell({ system, activeNav, isHome, isProjectHome, isProjectManagement, isGlobalSettings, onHome, onModelSettings, onSystemChange, onProjectChange, onProjectManage, onNavChange, children }: ShellProps) {
  ...
  const searchResults = useMemo(() => {
    ...
    return projects.flatMap(project => [
      { id: `${project.id}-project`, projectId: project.id, title: project.name, meta: `${project.brand} · Project`, projectHome: true as const },
    ...
    <header className="topbar">
      ...
      {!isGlobalSettings ? <div className="top-switcher-wrap">
        ...
        {projectMenu ? <div className="menu project-menu" role="menu">
          ...
          {projects.map(project => <button role="menuitem" key={project.id} className={project.id === currentProject.id ? 'project-choice' : 'menu-item'} onClick={() => { onProjectChange(project.id); setProjectMenu(false) }}><span className="project-code">{project.code}</span><span><b>{project.name}</b><small>{project.stage} · {project.updatedAt.slice(5)}</small></span>{project.id === currentProject.id ? <CheckCircle2 size={16}/> : null}</button>)}
          <button className="menu-link" role="menuitem" onClick={() => { onProjectChange(currentProject.id); setProjectMenu(false) }}>查看项目工作台</button>
