...
export function Shell({ system, activeNav, isHome, onHome, onSystemChange, onProjectChange, onNavChange, children }: ShellProps) {
  ...
  return <div className={`${isHome ? 'app-shell home-shell' : 'app-shell'}${collapsed && !isHome ? ' sidebar-collapsed' : ''}`}>
    ...
    <header className="topbar">
      ...
      <button className="icon-button" aria-label="命令中心"><Command size={18}/></button><button className="icon-button" aria-label="帮助"><CircleHelp size={18}/></button><button className="icon-button has-dot" aria-label="通知"><Bell size={18}/></button><button className="avatar" aria-label="个人菜单">AM</button>
    </header>
