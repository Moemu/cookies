...
import {
  api,
  buildHitAnalysisInput,
  buildLocalHitAnalysis,
  buildVideoReplicationPrompt,
  type ApiAdAccountBinding,
  type ApiAgencyWorkbench,
  type ApiArtifact,
  type ApiAssetFeature,
  type ApiAssetVersionPointer,
  type ApiCreativeSourceOption,
  type ApiGenerationJob,
  type ApiHitAnalysis,
  type ApiMaterialConfirmation,
  type ApiPreparedCommercePreroll,
  type ApiPrerollScope,
  type ApiProjectMediaAsset,
  type ApiQualityReport,
  type ApiRemixRenderJob,
  type ApiShortDramaHookStrategy,
  type ApiShortDramaPrerollCandidate,
  type ApiShortDramaPrerollPlan,
  type ApiShortDramaPrerollWorkspace,
  type ApiShortDramaStoryContext,
  type ApiTaskStrategyCreativeIntake,
  type ApiViralRemakeWorkspace,
  type ApiVideoPromptDimension,
  type ApiVideoReplicationPrompt,
} from '../data/api'
...
function CommerceHookWorkspace({ onNotice, handoffIntake }: { onNotice: (message: string) => void; handoffIntake?: ApiTaskStrategyCreativeIntake }) {
  ...
  const selectedProductAsset = inheritedProductAsset ?? selectedSource?.product.product_asset_refs[0]
