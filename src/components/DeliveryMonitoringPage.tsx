...
function recommendationTypeLabel(value: string) { return ({ review_compliance: '审核合规检查', tracking_review: '追踪链路检查', delivery_review: '跑量约束检查', cost_review: '成本控制检查', creative_test: '素材轮换测试', conversion_funnel_review: '转化漏斗检查', budget_pacing_review: '预算节奏检查' } as Record<string, string>)[value] ?? value }
function confidenceLabel(value: string) { return ({ low: '低置信度', medium: '中置信度', high: '高置信度' } as Record<string, string>)[value] ?? value }
function recommendationRationale(value: string) { return ({ 'Review the rejection reason or replace non-compliant material.': '检查审核拒绝原因，并替换不合规素材。', 'Check tracking before budget or bid changes.': '在调整预算或出价前，先检查转化追踪链路。', 'Review delivery constraints and use a controlled test.': '检查定向、出价和库存约束，并使用受控测试验证。', 'Review cost assumptions before increasing budget or bid.': '提高预算或出价前，先复核成本先验和成本边界。', 'Consider a controlled creative rotation test.': '考虑运行受控的素材轮换测试。', 'Check the conversion funnel before changing delivery settings.': '调整投放设置前，先检查转化漏斗。', 'Review budget pacing before changing the daily budget.': '调整日预算前，先检查预算消耗节奏。' } as Record<string, string>)[value] ?? value }
