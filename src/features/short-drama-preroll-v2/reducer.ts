import { fixtureImagePrompt, fixtureVideoPrompt } from './fixtures'
...
const clearImagesAndVideo = { imagesStatus: 'idle' as const, images: [], selectedImageId: '', ...clearVideo }
...
export function shortDramaPrerollReducer(state: ShortDramaPrerollState, action: ShortDramaPrerollAction): ShortDramaPrerollState {
  switch (action.type) {
    ...
    case 'hook-selected': {
      ...
      if (!hook || !state.analysis) return state
      return { ...state, selectedHookId: action.id, imagePrompt: fixtureImagePrompt(state.analysis, hook), videoDescription: hook.description, videoPrompt: fixtureVideoPrompt(state.analysis, hook, state.duration), activeStep: 'first-frame', ...clearImagesAndVideo }
    }
    case 'duration-changed': {
      const hook = state.hooks.find(item => item.id === state.selectedHookId)
      return { ...state, duration: action.duration, videoPrompt: state.analysis && hook ? fixtureVideoPrompt(state.analysis, hook, action.duration) : state.videoPrompt, ...clearVideo }
    }
    ...
    case 'image-selected': return { ...state, selectedImageId: action.id, activeStep: 'video', ...clearVideo }
