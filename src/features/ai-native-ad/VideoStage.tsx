...
export function VideoStage({ status, video, referenceFailure, onRetry, onFitVoiceover, voiceoverFitBusy = false, onCancel, onReviewReference }: VideoStageProps) {
  ...
  return <section>
    ...
    {referenceFailure ? <div className="video-reference-failure" role="alert"><span><ImageOff size={20}/></span><div><b>故事板参考图片未通过视频模型检查</b><p><strong>{referenceFailure.asset_name}</strong> 用于 {referenceFailure.unit_id}。{referenceFailure.reason}</p><small>{referenceFailure.asset_source === 'ai_generated' ? '请返回故事板重新生成这张图片，确认新图片后再一键成片。' : '这是商品链接导入图片，需要返回需求阶段更换合规商品图；原样重试仍会失败。'}</small></div><button className="secondary-button" type="button" onClick={() => onReviewReference(referenceFailure.asset_id)}><AlertTriangle size={13}/>查看问题素材</button></div> : null}
