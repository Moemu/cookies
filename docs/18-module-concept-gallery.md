...
- 后续页面不得照搬概念图中的多模块横向平铺。除洞察总览、投放作战台等信息展示页外，应强化一个主工作区，并将证据、版本、属性和日志渐进披露。
- 图表在保持坐标、单位、指标和置信信息清晰的前提下，需要通过转折点标注、素材关联、时间轴同步和品牌化线条节奏形成创意表达。
- 下一阶段应选择每个模块最重要的 1 至 2 个页面转成可交互原型，并逐项补全状态、权限和异常流程。

## 6. 生成提示词集

全部使用内置 Imagegen 的 `ui-mockup` 模式，并将已选定的智能蓝图方案图作为视觉参考。共同约束为：1440px 桌面端、矿物灰基底、钴蓝交互、深海军蓝文字、MiSans/Geist 气质、细边界、低圆角、层级清晰且密度克制；仅洞察和作战等信息展示页允许高密度。无移动端、无通用 AI 渐变、无玻璃拟态、无机器人和无巨型指标卡片。
