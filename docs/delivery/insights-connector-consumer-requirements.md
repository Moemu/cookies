...
> 2026-08-11：Delivery 当前唯一活跃配置运行时是不可变 `DeliveryIntent` + tagged `PlatformConfiguration`。本文提到的 ThreeTier 只表示历史差异分析；行为流程编译、Computer Use 与真实平台写入仍是未实现的未来边界。
...
智能投放拥有平台无关意图、目标平台配置、“投放账号上下文 → 项目 → 单元”的巨量语义、页面 schema 校准、预检、最终确认、执行证据、告警和建议。历史 ThreeTier 差异分析与未来页面字段/写入步骤由 Delivery 维护；Connector 只负责把获准读取的页面/API/导出数据变成可追溯事实。行为流程编译与受控写入必须另行立项，本文不表示这些能力已实现。
...
- **有限 Go**：会话级字段校准已在授权测试对象完成素材替换、项目启用/暂停和单元暂停/启用的可逆写后回读；这不授予上线后的 Computer Use 权限。
- **No-Go**：巨量平台创建/提交、项目排期、受输入约束阻塞的预算修改，以及任何缺少正式权限链的生产写入。
...
以下内容由对应 Connector 文件夹所有者在实现中冻结；它们不阻塞 Delivery 继续维护页面 Schema、历史 ThreeTier 差异分析和 mock 消费端口：
