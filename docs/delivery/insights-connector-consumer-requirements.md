...
智能投放拥有“投放账号上下文 → 项目 → 单元”的投放语义、页面 schema 校准、配置编译、预检、最终确认、受控写入、执行证据、告警和建议。页面字段的业务含义、依赖、ThreeTier 拆分和写入步骤由 Delivery 维护；Connector 只负责把获准读取的页面/API/导出数据变成可追溯事实。
