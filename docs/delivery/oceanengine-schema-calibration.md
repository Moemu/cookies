...
后续获得会话级可逆写入授权的 Phase D 字段校准记录保存在
[`oceanengine-d3-write-calibration-2026-08-16.json`](./evidence/oceanengine-d3-write-calibration-2026-08-16.json)；
该校准授权不创建上线后的 ChangeSet、Approval、Confirmation 或 ControlledActionAttempt。
...
单元出价没有跨场景统一下限：当前已观察到允许 0.01 元的路径，也有页面校验最低 4 元的展示量/CPM 路径。Computer Use 必须等待上游选择稳定后读取当前页面约束，并使用不超过当轮授权上限的页面最低值。产品主图优先沿用所选商品主图；没有商品主图时，当前“我的图片”有 1 个可选项，本地上传仅在获得上传授权时作为备选。
...
| `OE-M04`  | 应用 → 应用下载 → 安卓/iOS/鸿蒙      | 自动投放（UBMax）/手动投放入口均可见               | 三个平台均有唯一载体控件、应用引用与优化目标；iOS/鸿蒙经“使用该应用包”后均回读最大转化与手动投放，并显示竞价、金额、排期、监测和名称字段。安卓已有应用链路还观察到两条下载方式、应用选择器、项目额度、版本与过滤时间 | 安卓当前样本缺少可用事件目标；所有账户特定应用身份均已脱敏且未持久化；服务端校验和真实写后状态仍未授权 | `observed` + `blocked_by_event_asset`            |
| `OE-M05`  | 应用调起、预约下载、iOS、鸿蒙、直播 | 未提交项目表单                                     | 应用调起的安卓/iOS/鸿蒙及预约下载的安卓/iOS 已逐项回读；预约下载页明确将鸿蒙列为不支持，不是页面漂移 | 应用下载的非安卓合格事件资产、服务端校验和真实写后状态                                 | `observed` + `blocked_by_platform_capability`     |
...
| 优化目标 | 当前集合为表单提交、多转化、回访*信息确认、回访*高潜成交、私信留资中的四个可见候选以页面实际返回为准；本批实际选择“表单提交”形成连续路径     |
| 定向差异 | 新建/已有定向包、地域/性别/年龄/自定义人群、行为兴趣、过滤高活跃用户、过滤自己粉丝、过滤高关注用户、网络、已转化用户、智能定向拓展和手机价格 |
| 竞价     | 稳定成本、最大转化；日预算和出价输入可见                                                                                                     |

页面目标选择器本批实际列出“表单提交、多转化、回访*信息确认、回访*高潜成交、私信留资”五个文本项；其中目标数量和适用载体仍应作为动态集合，不写死为全局枚举。机器证据见 [`fixtures/oceanengine-leads-short-video-v0.1-draft.json`](./fixtures/oceanengine-leads-short-video-v0.1-draft.json)。
...
| 保存/开启/审核/错误     | 素材替换、项目启用/暂停和单元暂停/启用已在授权测试对象完成可逆写后回读             | 创建/提交、项目排期及受当前输入上限阻塞的预算写入继续保持 `write_validation_pending` |
...
因此页面业务校准可以关闭并冻结 `oceanengine-bidding-schema/v0.1`。它是当前账户与页面版本下的版本化观察契约，不宣称是巨量永久全量表单，也不授权上线后的真实执行。素材替换、项目启用/暂停和单元暂停/启用已有会话级可逆写入证据；创建/提交、项目排期、受输入上限阻塞的预算修改及其他未验证服务端校验继续标记为 `write_validation_pending`。
