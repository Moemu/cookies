...
| `OE-M04`  | 应用 → 应用下载 → 安卓/iOS/鸿蒙      | 自动投放（UBMax）/手动投放入口均可见               | 三个平台均有唯一载体控件、应用引用与优化目标；iOS 选择器可安全打开。安卓已有应用链路还观察到两条下载方式、应用选择器、项目额度、版本、过滤时间、监测链接组、预算和竞价 | 安卓当前样本缺少可用事件目标；iOS/鸿蒙未持久化任何账户特定应用身份，故不外推其资产后兼容链路 | `observed` + `blocked_by_event_asset`            |
