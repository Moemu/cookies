...
The control plane and deterministic fake path support two exact promotion
modification actions:

- `update_promotion_budget`
- `update_promotion_materials`

The 2026-08-14 read-only live inventory confirmed that schedule controls are on
the parent-project edit surface and appear only as inherited read-only summary
on the promotion edit surface. `update_promotion_schedule` is therefore invalid
for a promotion Mapping. A future schedule implementation must introduce a
confirmed project Mapping, project-owned state hashes and a separate authority
contract; it cannot reinterpret a promotion Mapping as project authority.
