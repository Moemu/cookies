...
- 四系统业务编辑页面必须选中一个 Project；“全部项目”仅用于跨项目工作台、搜索和待办聚合，不允许直接生成、审批或执行。
- 切换 Project 前保护未保存内容；切换后优先保留当前系统，并恢复目标 Project 在该系统的最近页面。
- Project 详情页不显示四个模块侧栏的混合版本；用户点击“继续工作”后才进入对应系统的 L1 导航。

详细数据边界、资源索引与权限见 [项目、品牌与产品域](./08-project-brand-domain.md)。
...
原“策略项目”统一更名为“策略工作区（StrategyWorkspace）”。全局 Project 连接四个系统；策略工作区只承载策略系统内部的对话、Brief、研究、策略、实验和评审，避免两个层级都叫“项目”。

### 3.2 完整导航树

| L1 一级导航 | L2 集合页或流程 | L3 对象内部视图 | 路由 |
| --- | --- | --- | --- |
| 策略工作台 | 我的待办、工作区进度、最近产物、质量与用量 | 无，工作台组件直接下钻 | `/strategy/home` |
| 策略工作区 | 全部、进行中、等待用户、待评审、已完成、归档 | 工作区详情见 3.3 | `/strategy/workspaces/*` |
...
打开 `/strategy/workspaces/:workspaceId/*` 后，L2 从列表状态切换为工作区流程；当前全局 `project_id` 保持不变：
...
默认进入“对话”，但首次进入未创建会话的工作区时进入“概览”。一个 Project 默认有一个主策略工作区，需要按市场、产品或阶段拆分时可以创建多个。
...
| 视频创作 | 效果广告、品牌广告、素材剪辑 | 品牌：Brief、剧本、资产、广告生成、剪辑；效果：爆款复刻、视频前贴、AI 数字人、AI 广告生成；剪辑：素材箱、多轨时间线、字幕、音频、导出 | `/creative/video/*` |
...
图文创作和视频创作必须保持 L1 平级；“品牌广告制作”“效果广告制作”“素材剪辑”保持视频创作下的固定 L2 入口。素材剪辑是通用制作工具，不作为第三种广告类型。

“视频草稿”和“视频版本”是任务、EditTask 与交付物的对象视图，不应作为与三类工作区平级的全局 L2 入口。
...
入口：`/creative/video/editing`

素材剪辑列表页的 L3 为：`全部任务、最近编辑、草稿、渲染中、待评审、已完成、失败`。进入 `/creative/video/editing/:editTaskId` 后不再显示一排功能标签，而是进入单一编辑器工作区：

| 区域 | 内容 |
| --- | --- |
| 顶栏 | 任务名称、保存状态、撤销重做、预览、版本、导出 |
| 左侧素材箱 | 上传素材、项目素材、AI 生成镜头、图片、音频、授权和代理状态 |
| 中央预览 | 比例、安全区、播放、画布及选中对象直接操控 |
| 右侧属性检查器 | 随选中对象显示画面、文字、字幕、音频、转场或基础效果参数 |
| 底部时间线 | 视频、叠加、字幕、配音、音乐和音效多轨时间线 |

字幕、音频、转场和效果是编辑器局部能力，不再拆成侧栏或 L3 导航。素材剪辑可独立创建 EditTask，也可以由品牌广告或效果广告任务携带素材和来源版本进入。详细规格见 [视频素材剪辑与开源框架方案](./21-video-material-editor-spec.md)。
...
```text
/projects/project_1001/overview
/projects/project_1001/artifacts
/strategy/workspaces/active
...
9. 图文、视频、数据分析和 Browser RPA 编辑器可以使用上下文面板，但不得把面板伪装成新的导航层级。
10. 用户可从 L0 进入 Project 总览并深链四系统；“全部项目”状态不能执行生成、审批或真实投放。
11. 策略系统只使用“策略工作区”命名和 `/strategy/workspaces/*` 新路由，不再把模块内流程容器称为 Project。
