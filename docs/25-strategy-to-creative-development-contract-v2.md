...
> 状态：MVP 冻结候选；三个 Schema 与四个 Golden Fixture 合入并通过 CI 后转为“已冻结”
>
> 日期：2026-07-28
>
...
- [ ] Strategy 同意在 Package 发布时冻结 Handoff，而不是读取时动态重建。
- [ ] 双方同意 Package Hash 与 Handoff Hash 的定义。
- [ ] 双方同意保留 `/creative-intakes` canonical path。
- [ ] 双方同意使用 `selected_route_id`，停止新增 `route_index` 调用方。
- [ ] 双方同意 Route 包含 deliverable、purpose、performance mode、channel 和 spec。
- [ ] 双方同意三级 readiness 及其操作门禁。
- [ ] 双方同意 Strategy readiness 仅作提示，Creative 本地结果是操作门禁。
- [ ] 双方同意业务信息不足创建 Intake，越权与 Hash 错误拒绝创建。
- [ ] 双方同意 Provider 密钥、真实模型和临时 URL 不进入业务契约。
- [ ] 三个 Schema、四个 Golden Fixture 和对应测试已合入。
