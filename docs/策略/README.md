# cookies 广告策略库

本目录将外部业务资料转化为可执行、可追溯、可验证的策略。内容服务于 cookies 的需求与策略、创意创作、素材洞察和智能投放四个系统。

## 阅读顺序

| 文档 | 解决的问题 | 主要落点 |
| --- | --- | --- |
| [01 素材洞察与能力路由策略](./01-素材洞察与能力路由策略.md) | 面对业务目标，应该调用哪类洞察、生成或达人能力 | 需求与策略、素材洞察 |
| [02 漫剧素材分析与制作策略](./02-漫剧素材分析与制作策略.md) | 漫剧题材如何选方向、拆素材、控制制作成本 | 素材洞察、创意创作 |
| [03 广告剪辑包装策略](./03-广告剪辑包装策略.md) | 如何用字幕、声音、转场、调色和贴纸提升可读性 | 创意创作、评审中心 |
| [04 商品与素材投放策略](./04-商品与素材投放策略.md) | 如何治理商品与素材组合，减少重复投放和素材疲劳 | 智能投放、素材洞察 |
| [05 素材全链路策略](./05-素材全链路策略.md) | 如何把洞察、创作、投放、复盘串成持续学习闭环 | 四系统共享 Project |

## 证据分级

- `来源事实`：可直接在学习资料中定位的数据、规则或案例描述。
- `策略推导`：基于多个来源事实形成的 cookies 执行方法。
- `待验证假设`：需要结合账户、题材、渠道或实验结果验证，不能当成平台硬规则。

所有比例、阈值和预算建议均保留来源语境。上线前应由业务负责人复核，并以真实账户数据校准。

## 学习来源

- [游戏行业创意图书馆](https://bytedance.sg.larkoffice.com/docx/Ch7pdSzpnoIfVSxJDRglRc3RgKh?from=from_copylink)
- [漫剧素材分析](https://bytedance.larkoffice.com/wiki/Omr6wJI8Qi3KWUkaanacFjwlnqT?from=from_copylink)
- [广告剪辑包装技巧](https://bytedance.larkoffice.com/wiki/BxrIwFC5qicnMckD29Yc1HycnKf)
- [素材投放策略](https://bytedance.larkoffice.com/wiki/JRqNw1YZWiu2q2kfLJicBn6Xndf?from=from_copylink)

最后学习日期：2026-07-22。
