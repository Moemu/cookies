...
Assets 通过注入的 Provider Output Fetcher 读取该不透明引用，并重新计算实际 MIME、大小与 SHA-256。Fetcher 的 `Open` 必须同时接收受信的 `ProjectRef` 与 `ProviderOutputRef`；Provider 的读取实现必须校验组织、项目、ProviderJob 与 output 归属，不能只凭输出 ID 或请求上下文推断项目范围。
