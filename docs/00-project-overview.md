...
| 文档版本 | v0.9 |
| 文档状态 | 草案 |
| 技术方向 | 四个独立垂直系统 + Golang/React 共享基座 + 火山引擎统一模型 Provider + ORAG RAG + Codex/Skills/Browser RPA |

## 1. 产品定义

cookies 是面向品牌方、代理商和增长团队的 AI 广告助手。需求与策略、创意创作、素材洞察、智能投放分别作为完整垂直系统建设；它们拥有独立导航、领域模型、权限和发布节奏，由全局 Project 连接同一业务闭环，并共享 Provider、知识库、用户组织、Codex/Skills、Browser RPA、任务与治理基座。
...
把分散在沟通、内容生产、素材数据和广告平台中的工作连接起来。Codex 负责理解、规划和调用能力，Skills 负责沉淀广告专业方法，Browser RPA 负责受控执行，人负责目标、品牌、合规、预算和最终决策。
...
5. 通过 Codex + Browser RPA 在人工审批下完成广告平台配置、上线、监控和优化。
...
- 不让 Browser RPA 绕过登录、验证码、平台审核、系统权限或人工审批。
...
| 广告投手/优化师 | 平台界面配置重复、监控压力大、复盘滞后 | Codex 规划、Browser RPA 执行并保留证据 |
...
9. **受控执行**：Browser RPA 操作已授权平台界面，在最终提交或敏感动作前暂停并请求人工确认。
...
| 广告智能投放 | 已批准计划、创意包、已登录平台会话 | Codex 规划、平台 Skills、Browser RPA 操作、人工审批、结果验证 | 平台侧投放、证据记录、告警、优化动作 |
...
| Browser RPA | 执行环境、允许应用/站点、截图、接管和安全策略 |
...
flowchart TB
  ...
  Insights["素材洞察系统"]
  Delivery["智能投放系统"]
  Platform["共享基座：用户、Provider、知识、Agent、Browser RPA、治理"]
...
7. **界面执行可观察**：Browser RPA 每一步都有目标、页面状态和结果证据，允许用户暂停或接管。
...
| 角色 | 默认权限 |
| --- | --- |
| 组织管理员 | 组织设置、成员、Skills、Browser RPA 允许范围、模型与数据策略 |
...
权限采用组织、项目、资源三级作用域。cookies 不保存用户广告账户密码或验证码，Browser RPA 使用用户已登录且允许访问的会话；如接入只读数据连接器，其令牌由服务端加密保存。上线、调预算和删除动作需独立权限。
...
| 共享管理台 | `/admin/*` | 用户组织、Provider、知识库、Agent、Browser RPA、工作流、安全、用量 |
...
- 共享平台包：身份、Provider、知识、Agent、Browser RPA、任务、审批、通知和审计。
...
- Browser RPA 协调层：管理目标应用/站点白名单、人工接管、关键动作确认、截图/页面证据和运行超时。
...
- 投放设备、浏览器会话、接管和证据见 [Browser RPA 运行时](./12-browser-rpa-runtime.md)。
...
- Browser RPA 只可访问用户/管理员允许的应用和站点；登录、验证码、支付、权限、账户和预算操作必须保持人工在环。
...
- 建立全局壳层、四个系统导航骨架和共享基座；完成用户组织、Provider、知识、Codex Task、Skill 注册、Browser RPA 与审计。
- 用人工服务 + 原型验证完整闭环和字段模型。
...
- 通过一个首发平台 Skill + Browser RPA 完成草稿、校验、审批、上线和结果验证。
- 建立核心埋点、质量评测和错误监控。
...
4. Browser RPA 在提交前展示目标、预算、对象和页面状态；重复运行不会重复创建未知对象。
5. 核心操作具备审计日志，账户凭据不出现在前端、日志或错误信息中。
6. Codex、Skill、Browser RPA 或平台页面失败均有可理解的状态、证据和恢复路径。
...
| Browser RPA 误操作造成预算损失 | 允许列表、预算上限、关键点击审批、截图证据、用户接管和暂停 |
...
1. 首个试点行业、地区和主要客户类型。
2. 首发 Browser RPA 平台 Skill；建议按客户消耗在抖音和快手中选择首个受控试点。
