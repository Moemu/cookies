...
func (r CreateReportRequest) Validate() error {
	if strings.TrimSpace(r.ExecutionID) == "" || strings.TrimSpace(r.Summary) == "" ||
		len(r.Summary) > 2000 || len(r.Findings) < 1 || len(r.Findings) > 20 {
...
func (s Service) CreateReport(ctx context.Context, actor contract.ActorContext, projectID contract.ProjectID, request CreateReportRequest) (InsightReport, error) {
	...
	return s.Repository.CreateReport(ctx, InsightReport{
		...
		EvidenceSummary: execution.EvidenceSummary, Status: ReportDraft,
		Summary: strings.TrimSpace(request.Summary), Findings: append([]string{}, request.Findings...),
