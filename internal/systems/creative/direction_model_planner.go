...
func (p ModelCreativeDirectionPlanner) Generate(
	...
	if planningContext.SelectedRoute.RouteType == CreativeRouteBrandVideo {
		systemPrompt += "当前路线是品牌视频。目标不是教程或转化清单，而是建立可复述的品牌认知与感受。" +
			...
			"三个候选应分别探索不同品牌领地，例如人物情绪弧、工程世界的视觉隐喻、品牌仪式或记忆符号；开场可以渠道原生，但不能以促销 CTA 或知识教学主导。品牌片结尾只允许回到品牌主张或品牌身份，不得使用点击、评论、私信、领取、扣1、立即咨询等效果 CTA。" +
			"不得擅自增加 4K、8K 等输入未确认的制作规格。emotional 或 cinematic 候选的概念和理由中不得出现指南、清单、三步、避坑、工具、教程、科普、方法论。" +
	...
	for attempt := 0; attempt < 2; attempt++ {
		...
		response, err := p.Text.GenerateText(ctx, provider.TextGenerateRequest{
			...
			InvocationKey:    contract.IdempotencyKey(fmt.Sprintf("direction_%s_%d_v3_%d", hashToken, candidateCount, attempt)),
			Messages:         messages,
