...
var controlledActionBatchOrder = []string{"update_promotion_budget", "update_promotion_materials", "pause_promotion", "resume_promotion"}
var controlledActionBatchExpected = []ControlledActionCapability{
...
type ControlledActionCapability struct {
	Action                 string `json:"action"`
	CalibrationStatus      string `json:"calibration_status"`
	ControlledActionStatus string `json:"controlled_action_status"`
}
...
type Definition struct {
	...
	RuntimePolicy struct {
		...
		ControlledActionBatchLiveCalibrated    bool   `json:"controlled_action_batch_live_calibrated"`
	...
	ControlledActionBatchCalibration struct {
		ObservedAt                         string                       `json:"observed_at"`
		EvidenceRef                        string                       `json:"evidence_ref"`
		MappingRevision                    int64                        `json:"mapping_revision"`
		MappingStatus                      string                       `json:"mapping_status"`
		MappingPlatformStatusSnapshot      string                       `json:"mapping_platform_status_snapshot"`
		OrderedActions                     []string                     `json:"ordered_actions"`
		CapabilityMatrix                   []ControlledActionCapability `json:"capability_matrix"`
		MaximumRemoteWriteClicksAuthorized int                          `json:"maximum_remote_write_clicks_authorized"`
		ActualRemoteWriteClicks            int                          `json:"actual_remote_write_clicks"`
		...
		LiveRemoteModificationAllowed      bool                         `json:"live_remote_modification_allowed"`
	} `json:"controlled_action_batch_calibration"`
}
...
func (d Definition) Validate() error {
	...
	if d.SchemaVersion != "delivery-platform-skill-definition/v1" ||
		...
		!d.RuntimePolicy.ControlledActionBatchLiveCalibrated ||
		...
		d.ControlledActionBatchCalibration.ObservedAt != "2026-08-14" ||
		d.ControlledActionBatchCalibration.EvidenceRef != "docs/delivery/evidence/oceanengine-controlled-actions-batch-2026-08-14.json" ||
		d.ControlledActionBatchCalibration.MappingRevision != 2 ||
		d.ControlledActionBatchCalibration.MappingStatus != "confirmed" ||
		d.ControlledActionBatchCalibration.MappingPlatformStatusSnapshot != "pending_review" ||
		!slices.Equal(d.ControlledActionBatchCalibration.OrderedActions, controlledActionBatchOrder) ||
		!slices.Equal(d.ControlledActionBatchCalibration.CapabilityMatrix, controlledActionBatchExpected) ||
		d.ControlledActionBatchCalibration.MaximumRemoteWriteClicksAuthorized != 2 ||
		d.ControlledActionBatchCalibration.ActualRemoteWriteClicks != 0 ||
		d.ControlledActionBatchCalibration.ControlledActionAttemptCount != 0 ||
		d.ControlledActionBatchCalibration.FinalConfirmationCount != 0 ||
		d.ControlledActionBatchCalibration.RunTerminalState != "cancelled" ||
		d.ControlledActionBatchCalibration.StopReason != "lease_expired_before_final_confirmation" ||
		d.ControlledActionBatchCalibration.RemoteWriteDetected ||
		d.ControlledActionBatchCalibration.LiveRemoteModificationAllowed ||
