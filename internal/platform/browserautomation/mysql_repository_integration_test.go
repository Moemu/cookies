...
func TestMySQLAuthorizeControlledActionIsAtomic(t *testing.T) {
	...
	if _, err = db.ExecContext(ctx, `INSERT INTO organizations (id,name,status) VALUES (?,?,'active')`, org, "browser-rpa integration"); err != nil {
		t.Fatal(err)
	}
	if _, err = db.ExecContext(ctx, `INSERT INTO projects (id,organization_id,name,status) VALUES (?,?,?,'draft')`, project, org, "browser-rpa integration"); err != nil {
		t.Fatal(err)
	}
	defer func() {
		for _, statement := range []string{`DELETE FROM browser_rpa_controlled_action_attempts WHERE organization_id=?`, `DELETE FROM browser_rpa_final_confirmations WHERE organization_id=?`, `DELETE FROM browser_rpa_evidence WHERE organization_id=?`, `DELETE FROM browser_rpa_events WHERE organization_id=?`, `DELETE FROM browser_rpa_run_steps WHERE organization_id=?`, `UPDATE browser_rpa_runs SET lease_id=NULL WHERE organization_id=?`, `DELETE FROM browser_rpa_session_leases WHERE organization_id=?`, `DELETE FROM browser_rpa_runs WHERE organization_id=?`, `DELETE FROM browser_rpa_site_policies WHERE organization_id=?`, `DELETE FROM browser_rpa_browser_profiles WHERE organization_id=?`, `DELETE FROM browser_rpa_environments WHERE organization_id=?`, `DELETE FROM projects WHERE organization_id=?`, `DELETE FROM organizations WHERE id=?`} {
			_, _ = db.ExecContext(context.Background(), statement, org)
		}
	}()
	_, err = db.ExecContext(ctx, `INSERT INTO browser_rpa_environments (id,organization_id,project_id,platform,account_id,mode,browser_version,region,healthy,version) VALUES (?,?,?,'ocean_engine','account_1','local_visible','test','local',TRUE,1)`, env, org, project)
	if err != nil {
		t.Fatal(err)
	}
	_, err = db.ExecContext(ctx, `INSERT INTO browser_rpa_browser_profiles (id,organization_id,project_id,environment_id,platform,account_id,state,version) VALUES (?,?,?,?,'ocean_engine','account_1','ready',1)`, profile, org, project, env)
	if err != nil {
		t.Fatal(err)
	}
	_, err = db.ExecContext(ctx, `INSERT INTO browser_rpa_site_policies (id,organization_id,project_id,platform,account_id,allowed_protocols,allowed_hosts,allowed_page_kinds,allowed_platform_project_ids,version) VALUES (?,?,?,'ocean_engine','account_1',JSON_ARRAY('https'),JSON_ARRAY('example.test'),JSON_ARRAY('review'),JSON_ARRAY('platform_project_1'),1)`, policy, org, project)
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err = db.ExecContext(ctx, `INSERT INTO browser_rpa_run_steps (id,organization_id,project_id,run_id,sequence_number,workflow_step_id,action,status,attempt,version) VALUES (?,?,?,?,1,'submit','submit_platform_configuration','pending',1,1)`, stepID, org, project, run.ID); err != nil {
		t.Fatal(err)
	}
	...
	if err = db.QueryRowContext(ctx, `SELECT COUNT(*),MAX(consumed_at) FROM browser_rpa_controlled_action_attempts a JOIN browser_rpa_final_confirmations c ON c.id=a.confirmation_id WHERE a.organization_id=? AND a.run_id=?`, org, run.ID).Scan(&attempts, &consumed); err != nil {
		t.Fatal(err)
	}
	...
	if err = db.QueryRowContext(ctx, `SELECT status FROM browser_rpa_controlled_action_attempts WHERE organization_id=? AND project_id=? AND id=?`, org, project, authorized.Attempt.ID).Scan(&takeoverAttemptStatus); err != nil || takeoverAttemptStatus != ControlledActionResultUnknown {
		t.Fatalf("attempt status=%q err=%v", takeoverAttemptStatus, err)
	}
	var takeoverEvidence, takeoverEvents, takeoverSteps int
	if err = db.QueryRowContext(ctx, `SELECT (SELECT COUNT(*) FROM browser_rpa_evidence WHERE organization_id=? AND run_id=?),(SELECT COUNT(*) FROM browser_rpa_events WHERE organization_id=? AND run_id=?),(SELECT COUNT(*) FROM browser_rpa_run_steps WHERE organization_id=? AND run_id=?)`, org, takeoverRun.ID, org, takeoverRun.ID, org, takeoverRun.ID).Scan(&takeoverEvidence, &takeoverEvents, &takeoverSteps); err != nil {
		t.Fatal(err)
	}
