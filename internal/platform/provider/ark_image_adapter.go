...
func (a *ArkImageAdapter) Submit(ctx context.Context, request ImageGenerationRequest) (ImageSubmission, error) {
	...
	if err != nil {
		return ImageSubmission{}, fmt.Errorf("encode %s image request: %w", a.providerName, err)
	}
	...
	for index, result := range decoded.Data {
		contents, decodeErr := base64.StdEncoding.DecodeString(result.B64JSON)
		if decodeErr != nil || len(contents) == 0 || len(contents) > arkImageMaxBytes {
...
func newCompatibleImageAdapter(apiKey, model, baseURL, endpointPath, providerCode, providerName string, handles OutputHandleStore) (*ArkImageAdapter, error) {
	...
	return &ArkImageAdapter{apiKey: apiKey, model: model, baseURL: strings.TrimRight(baseURL, "/"), endpointPath: endpointPath, providerCode: providerCode, providerName: providerName, client: &http.Client{Timeout: 45 * time.Second}, handles: handles, now: time.Now}, nil
}
