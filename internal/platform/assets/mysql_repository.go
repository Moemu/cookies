...
const projectAssetSelect = `SELECT pa.project_id, pa.created_at, a.id, a.organization_id, a.asset_kind, a.status, a.owner_system, a.latest_version, a.created_at, a.updated_at, av.version, av.status, av.source_type, av.mime_type, av.size_bytes, av.sha256, av.width_pixels, av.height_pixels, av.duration_ms, av.frame_rate, av.video_codec, av.audio_codec, av.render_job_id, av.derivation_id, av.duration_seconds, av.fps, av.codec, av.bitrate_bps, av.audio_codec, av.audio_channels, av.audio_sample_rate, av.poster_frame_ref, av.probe_status, av.probe_error, av.provider_job_id, av.provider_output_id, av.ocean_engine_material_id, av.project_context_version, av.created_at, b.storage_provider, b.bucket_name, b.object_key, b.storage_version_id, b.etag FROM project_assets pa JOIN assets a ON a.organization_id=pa.organization_id AND a.id=pa.asset_id JOIN asset_versions av ON av.organization_id=pa.organization_id AND av.asset_id=pa.asset_id AND av.version=pa.asset_version JOIN asset_blobs b ON b.id=av.blob_id`
...
func scanProjectAsset(row scanner) (ProjectAsset, error) {
	...
	var frameRate, videoCodec, audioCodec, renderJob, derivationID, codec, mediaAudioCodec, posterFrame, probeStatus, probeError, job, output, oceanEngineMaterialID, versionID, etag sql.NullString
	var contextVersion sql.NullInt64
	err := row.Scan(&v.Ref.ProjectID, &v.CreatedAt, &v.Asset.ID, &v.Asset.OrganizationID, &v.Asset.Kind, &v.Asset.Status, &v.Asset.OwnerSystem, &v.Asset.LatestVersion, &v.Asset.CreatedAt, &v.Asset.UpdatedAt, &v.Version.Version, &v.Version.Status, &v.Version.SourceType, &v.Version.MIMEType, &v.Version.SizeBytes, &v.Version.SHA256, &width, &height, &duration, &frameRate, &videoCodec, &audioCodec, &renderJob, &derivationID, &mediaDuration, &fps, &codec, &bitrate, &mediaAudioCodec, &channels, &sampleRate, &posterFrame, &probeStatus, &probeError, &job, &output, &oceanEngineMaterialID, &contextVersion, &v.Version.CreatedAt, &v.Version.Blob.Provider, &v.Version.Blob.Bucket, &v.Version.Blob.Key, &versionID, &etag)
	if errors.Is(err, sql.ErrNoRows) {
	...
	v.Version.OceanEngineMaterialID = oceanEngineMaterialID.String
