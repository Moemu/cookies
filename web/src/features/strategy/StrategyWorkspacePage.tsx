...
function StrategyPane({ draft, review, packageVersion, busy, canGenerate, onGenerate, onPatch, onSubmit, onApprove }: {
  ...
  return <article className="strategy-panel strategy-document">
    ...
    <footer className="strategy-review-actions">
      {packageVersion ? <div className="published-proof"><strong>策略包 v{packageVersion.version} 已发布</strong><code>{packageVersion.content_hash}</code></div> : review?.status === 'open' ? <>
