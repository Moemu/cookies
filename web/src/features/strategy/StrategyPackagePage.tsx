  ...
  return <section className="strategy-page strategy-object-page">
    ...
    <div className="package-layout">
      ...
      <aside className="package-sidebar">
        ...
        <section><h2>显式交接</h2><p>Creative 首期只接收小红书只读投影。点击后才会创建 Intake。</p><button className="button button--primary" disabled={busy || !value.snapshot.readiness.creative_ready} onClick={async () => {
          setBusy(true)
          setError('')
          try {
        ...
        }} type="button">发送到 Creative</button>{!value.snapshot.readiness.creative_ready ? <small>策略包未包含可交接的小红书执行方案。</small> : null}</section>
