...
import { approveCreativeVersion, archiveCreativeTask, bindCreativeImageAsset, checkCreativeVersion, createImagePlanJob, createCreativeIntake, createCreativeTask, deliverCreativeVersion, freezeCreativeVersion, getCreativeTask, listCreativeIntakes, listCreativePackages, listCreativeTasks, listCreativeVersions, reviseCreativeDraft } from './api'
...
export function CreativeImageTextPage() {
  const { projectId = '', taskId = '', creativeStage = 'content' } = useParams()
  const navigate = useNavigate()
  const location = useLocation()
  const activeStage: CreativeStage = creativeStages.some((stage) => stage.key === creativeStage) ? creativeStage as CreativeStage : 'content'
  ...
  const selectTask = useCallback(async (taskId: string, signal?: AbortSignal) => {
    const [detail, history] = await Promise.all([
      getCreativeTask(projectId, taskId, signal),
      Promise.allSettled([
        listCreativeVersions(projectId, taskId, signal),
        listCreativePackages(projectId, signal),
      ]),
    ])
    setSelected(detail)
    const [versions, packages] = history
    const latestVersion = versions.status === 'fulfilled' ? versions.value.items[0] ?? null : null
    setFrozenVersion(latestVersion)
    setCreativePackage(latestVersion && packages.status === 'fulfilled'
      ? packages.value.items.find((item) => item.creative_version_id === latestVersion.id) ?? null
      : null)
    if (!signal?.aborted) setError('')
  }, [projectId])

  const openTask = useCallback(async (nextTaskId: string, stage: CreativeStage = 'content') => {
    await selectTask(nextTaskId)
    if (location.pathname.includes('/creative/tasks')) {
      navigate(creativeTaskPath(projectId, nextTaskId, stage))
    }
  }, [location.pathname, navigate, projectId, selectTask])
  ...
  useEffect(() => {
    if (!taskId || creativeStages.some((stage) => stage.key === creativeStage)) return
    navigate(creativeTaskPath(projectId, taskId, 'content'), { replace: true })
  }, [creativeStage, navigate, projectId, taskId])
  ...
  useEffect(() => {
    ...
    const refreshProduction = async () => {
      try {
        ...
        const relatedAssets = assets.items.filter((asset) => jobIds.includes(asset.version.provider_job_id ?? ''))
        const previews = await Promise.all(relatedAssets.map(async (asset) => {
          try {
            const preview = await getAssetPreview(projectId, asset.asset.id, asset.version.version, controller.signal)
            return [asset.asset.id, preview.url] as const
          } catch (caught) {
            if (caught instanceof DOMException && caught.name === 'AbortError') throw caught
            return [asset.asset.id, ''] as const
          }
        }))
        if (controller.signal.aborted) return
        setProviderJobs(Object.fromEntries(jobs.map((job) => [job.id, job])))
        setProductionAssets(relatedAssets)
        setPreviewUrls(Object.fromEntries(previews.filter(([, url]) => Boolean(url))))
        setProductionError('')
      } catch (caught) {
        if (caught instanceof DOMException && caught.name === 'AbortError') return
        if (!controller.signal.aborted) setProductionError(caught instanceof Error ? caught.message : '无法读取生产任务状态。')
      }
    }
  ...
  async function createTaskFromIntake(intake: CreativeIntake, direction: CreateCreativeTaskInput) {
    ...
    try {
      ...
      await openTask(task.id, 'content')
  ...
  async function archiveSelectedTask() {
    ...
    try {
      ...
      if (remaining.length > 0) await openTask(remaining[0].id)
      else {
        setSelected(null)
        if (location.pathname.includes('/creative/tasks')) navigate(creativeTasksPath(projectId))
      }
  ...
  const previewAsset = currentProductionAssets.find((asset) => previewUrls[asset.asset.id])
  const previewUrl = previewAsset ? previewUrls[previewAsset.asset.id] : ''
  ...
  return <section className="creative-page">
    ...
    <div className="creative-workspace" id="creative-intake-workspace">
      ...
      <aside className="creative-task-list" aria-label="策略接入与创意任务" id="creative-task-queue">
        ...
        <section className="creative-handoff-section" aria-label="图文任务">
          ...
          {loading ? <p className="creative-empty">正在加载任务…</p> : tasks.length === 0 ? <p className="creative-empty">尚未创建图文任务。完成策略交接或手工输入后，初稿会显示在这里。</p> : <div className="creative-task-items">{tasks.map((task) => {
            ...
            return <button className={selected?.task.id === task.id ? 'creative-task-item creative-task-item--selected' : 'creative-task-item'} key={task.id} onClick={() => void openTask(task.id)} type="button"><span>{task.direction.focus || task.direction.concept || source?.request.core_message || '未命名方向'}</span><small>{taskTypeLabels[task.direction.content_type] ?? '图文方向'} · {task.channel === 'xiaohongshu' ? '小红书图文' : task.channel} · {task.status === 'draft' ? '初稿' : task.status}</small><em>{reference ? `来自策略包 v${reference.package_version}` : '手工创意输入'}</em></button>
          })}</div>}
        </section>
      </aside>
    </div>
    ...
    {selected ? <section className="creative-draft" aria-label="图文内容初稿" id="creative-production">
      <nav className="creative-task-breadcrumb" aria-label="创意任务面包屑">
        <Link to={creativeTasksPath(projectId)}>创意任务</Link><span>/</span><span>{selected.task.direction.focus || selected.task.direction.concept || selected.task.id}</span>
      </nav>
      ...
      <ol className="creative-task-stages" aria-label="创意任务阶段">
        {creativeStages.map((stage, index) => <li key={stage.key}>
          <Link
            aria-current={stage.key === activeStage ? 'step' : undefined}
            className={stage.key === activeStage ? 'creative-task-stage creative-task-stage--active' : 'creative-task-stage'}
            onClick={() => {
              const target = stage.key === 'production' ? 'creative-production-workspace' : stage.key === 'check' || stage.key === 'review' || stage.key === 'delivery' ? 'creative-review' : 'creative-production'
              window.requestAnimationFrame(() => document.getElementById(target)?.scrollIntoView({ behavior: 'smooth', block: 'start' }))
            }}
            to={creativeTaskPath(projectId, selected.task.id, stage.key)}
          >
            <b>{String(index + 1).padStart(2, '0')}</b><span>{stage.label}</span><small>{stage.description}</small>
          </Link>
        </li>)}
      </ol>
      {['check', 'review', 'delivery'].includes(activeStage) && frozenVersion?.creative_task_id !== selected.task.id ? <div className="creative-stage-notice" role="status">
        <div><strong>尚未加载可评审的冻结版本</strong><span>当前后端还没有“按任务读取 CreativeVersion”的查询契约。请先在本次页面会话中冻结草稿；刷新后系统不会伪造版本状态。</span></div>
        <Link className="text-link" to={creativeTaskPath(projectId, selected.task.id, 'content')}>返回内容阶段</Link>
      </div> : null}
      ...
      <div className="creative-draft__grid" id="creative-production-workspace">
        <article className="creative-copy"><h3>标题候选</h3><ol>{selected.draft.title_candidates.map((title) => <li key={title}>{title}</li>)}</ol><h3>正文</h3><p>{selected.draft.body}</p><div className="creative-topics">{selected.draft.topics.map((topic) => <span key={topic}>{topic}</span>)}</div></article>
        <article className="creative-preview" aria-label="图文视觉预览">
          <div className="creative-preview__toolbar"><strong>{previewUrl ? '已入库视觉预览' : '封面排版预览'}</strong><small>{previewAsset ? `${previewAsset.version.width_pixels ?? '—'} × ${previewAsset.version.height_pixels ?? '—'}` : '小红书图文'}</small></div>
          <div className={previewUrl ? 'creative-cover__canvas creative-cover__canvas--image' : 'creative-cover__canvas'}>
            {previewUrl ? <img alt="当前图文已入库素材预览" src={previewUrl} /> : <><span>封面文字</span><strong>{selected.draft.cover_copy}</strong><small>{selected.task.direction.tone.join(' · ') || '小红书图文'}</small></>}
          </div>
          <div className="creative-preview__caption"><span>当前画面</span><strong>{selected.draft.cover_copy}</strong><small>{previewAsset ? `Asset ${previewAsset.asset.id} · v${previewAsset.version.version}` : '生成完成并入库后，将自动显示真实图片。'}</small></div>
        </article>
        <article className="creative-cover"><div className="creative-inspector__heading"><div><span>生成配置</span><h3>图组结构与提示词</h3></div><small>Provider</small></div><ol className="creative-image-plan">{selected.draft.image_plan.map((item) => {
    ...
    {productionError ? <div className="creative-production-warning" role="status"><div><strong>生产状态暂时无法刷新</strong><span>{productionError}</span></div><small>页面会自动重试，不影响已经提交的 Provider 作业。</small></div> : null}
